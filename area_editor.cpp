...
void AreaEditor::fill(Terrain t) {
    for (int j = 0; j < m_area->get_height(); j++) {
        hline(j, Range(0, m_area->get_width()), t);
    }
}

void AreaEditor::hline(int y, const Range &xrange, Terrain t) {
    for (int i = xrange.min; i < xrange.max; i++) {
        m_area->set_terrain(Pos(i, y), t);
    }
}

void AreaEditor::vline(int x, const Range &yrange, Terrain t) {
